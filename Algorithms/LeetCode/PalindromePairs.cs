...
namespace Algorithms.LeetCode
{
    ...
    public class PalindromePairs : Runable
    {
        ...
        protected override object DoAction()
        {
            ...
            {
                var value = PalindromePairs2(words);
        ...
        public IList<IList<int>> PalindromePairs2(String[] words)
        {
            ...
            for (int i = 0; i < words.Length; i++)
            {
                for (int j = 0; j <= words[i].Length; j++)
                {
                    ...
                    {
                        string str2rvs = new string(str2.Reverse().ToArray());

                        if (map.ContainsKey(str2rvs) && map[str2rvs] != i)
                        {
                            List<int> list = new List<int>();
                            list.Add(map[str2rvs]);
                            list.Add(i);
                            ret.Add(list);
                        }
                    }
                    ...
                    {
                        string str1rvs = new string(str1.Reverse().ToArray());
                        // check "str.Length() != 0" to avoid duplicates
                        if (map.ContainsKey(str1rvs) && map[str1rvs] != i && str2.Length != 0)
                        {
                            List<int> list = new List<int>();
                            list.Add(i);
                            list.Add(map[str1rvs]);
                            ret.Add(list);
                        }
                    }
                }
